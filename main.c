...
//Helper functions declaration for further use (Their description is listed below)
void monsterMoves(int** wumpusMaze);
void ricochet(int *currentArrowLocation, int *currentPlayerLocation, int *currentWumpusLocation);
void fatality(int* x);


//Done => Fills the Wumpus Maze with the contents using malloc() => HARDCODE the instructions (Assignment's description) => Office Hours
...
//Done => helper function for producing output: check if the player's location == to the pit's or Wumpus' locations
//If so => proceed to the next phase depending on the condition
void nextLevel(int** wumpusMaze, size_t* mazeRows){
...
//Done => driver function to complete the Shoot(). 
//Using the provided arguments, it detects if the currentArrowLocation == currentWumpusLocation
//Or currentPlayerLocation, and then kills one or another depending on the conditions
void kill(int* battlefield, int shots, bool guess, int** wumpusMaze){

    for(int i = 0; i < shots; i++){
            for(int j = 0; j < 4; j++){
                if(battlefield[i] == wumpusMaze[currentArrowLocation - 1][j]){
                    currentArrowLocation = battlefield[i]; //Now the arrow is aiming to the selected location on the battlefield
                    int x = currentArrowLocation;
                    guess = true; //The rooms are adjacent
                    fatality(&x);
                }
            }
            ...
            if(guess == false){
                ...
                currentArrowLocation = wumpusMaze[currentArrowLocation - 1][1];
                killshot(&x);
                ricochet(&currentArrowLocation, &currentPlayerLocation, &currentWumpusLocation);
                guess = false;
            }
...
//Done => if the arrow ricochets => compare currentArrowLocation with currentPlayerLocation or currentWumpusLocation, and, if they are equal => kill player or Wumpus
void ricochet(int *currentArrowLocation, int *currentPlayerLocation, int *currentWumpusLocation){
...
void Shoot(int* moveNumber, int** wumpusMaze, int* battlefield){
    
    //Remember arrowRoom value of -1 means it is with the person.
    if(currentArrowLocation == -1){
        ...
        kill(battlefield, shots, guess, wumpusMaze);
        currentWumpusLocation = wumpusMaze[currentWumpusLocation - 1][1];
        moveNumber++;   
    }
}
...
//Done: randomly sets all game elements on the map => everything must go respectively: player, wumpus, pits, and bats
...
void monstaLocations(void){
    ...
    }while(currentWumpusLocation == currentPit2Location || currentWumpusLocation == currentPit1Location || currentWumpusLocation == currentPlayerLocation || currentWumpusLocation == currentBat1Location || currentWumpusLocation == currentBat2Location);
    ...
    switch(choice){
        ...
        case 'S':
            if(currentArrowLocation != -1){
                ...
                int* battlefield = malloc(3 * sizeof(int));
                Shoot(moveNumber, wumpusMaze, battlefield); //Must be a list of numbers separated by spaces
                printf("You are in room %d", currentPlayerLocation);
                printf(".");
                printf("\n\n");
                (*moveNumber)++;
